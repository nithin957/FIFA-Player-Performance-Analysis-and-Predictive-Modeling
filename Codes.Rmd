...
fifa_df <- read_csv("outputplayers20.csv")
summary(fifa_df)
str(fifa_df)
```
...
table_1<- knitr::kable(na_summary_filtered, align = "c", caption = "Table 1: Columns with Null Values") %>%
...
mean_overall_df <- data.frame(
  "Mean Overall Ratings for Europian Players" =mean_overall_europe ,
  "Mean Overall Ratings for South American Players" = mean_overall_south_america
)

table_2<-knitr::kable(mean_overall_df, align = "c",
                        caption = "Table 2: Mean Overall Ratings for Different Regions") %>%
    kable_classic(full_width = FALSE)
table_2
```
...
In this model, we aim to develop a predictive tool for assessing players' overall potential. Factors such as physique, age, and performance across various levels will be considered. Before applying regularization techniques to construct the model, it is imperative to address variables showing significant collinearity or multicollinearity with other predictors.

Collinearity denotes the linear association among two or more predictors in a regression model, while multicollinearity arises when predictors are strongly correlated, hindering the estimation of individual predictor effects on the target variable *(Songara,2022)*.

During exploratory data analysis (EDA), variables demonstrating high collinearity or multicollinearity are identified through correlation matrices and variance inflation factor (VIF). Consequently, one variable from each pair is removed.
```{r}
...
The data frame contains numerous non numeric variables so, selecting only numeric variables for prediction. We will be using only numeric columns to build regularization model.So, plotting the correlation matrix for numeric columns to remove higly correlated variables.

```{r fig.align='center', fig.cap='Fig.7:Correlation Matrix for Numeric Variables', message=FALSE, warning=FALSE}
fifa_df_BQ2_num<- select_if(fifa_df_BQ2, is.numeric)

# Calculate the correlation matrix for numeric columns
corr_matrix_BQ2 <- cor(fifa_df_BQ2_num, use = "pairwise.complete.obs")

corrplot(corr_matrix_BQ2, method = "color", type = "lower",
         tl.col = "black", tl.srt = 45,
         tl.cex = 0.7, cl.cex = 0.7, number.cex = 0.7,
         col = colorRampPalette(c("#FFFFFF", "#4575b4", "#a0a0a0", "#d73027", "#ffffbf"))(200))


#Finding columns with high collinearity
high_collinear_BQ2 <- findCorrelation(corr_matrix_BQ2, cutoff = 0.9)

#Removing highly collinear columns from the dataframe
fifa_df_BQ2_num <- fifa_df_BQ2_num[, -high_collinear_BQ2]
```

We will now be splitting the data set into train and test data set to build model.We split the dataset into training and testing sets to develop predictive models on the training data and evaluate their performance on unseen data, ensuring the model's generalization and estimating its effectiveness in real-world scenarios *(Casati,2022)*. We will be splitting dataset into 80 and 20 ratio so that model is trained on 80% of the dataset and tested on the remaining 20% of the dataset. 
...
To construct a predictive model for assessing overall potential, our initial approach involves implementing ridge regularization. This technique, by introducing a penalty term to the regression coefficients, helps mitigate multicollinearity and overfitting, thus enhancing model generalization and robustness. Subsequently, we will evaluate the predictive model's performance by examining the Root Mean Squared Error (RMSE) values. 

## Ridge Regularization Model:
```{r fig.align='center', fig.cap='Fig.8:Ridge-Cross Validation Plot', message=FALSE, warning=FALSE}
set.seed(354)
...
plot(ridge_BQ2)
text(log(ridge_BQ2$lambda.min), max(ridge_BQ2$cvm)/1.2, 
     labels ="log λ.Min",
     pos = 1,srt=90,cex=0.8, col = "red")  

text(log(ridge_BQ2$lambda.1se), max(ridge_BQ2$cvm)/1.2, 
     labels ="log λ.1se", 
     pos = 1, col = "red",cex=0.8, srt=90)
```

It can be observed that the number of terms in Ridge model are 41 which is making the model complex. Also, the lambda values are quite close to each other. We can try to fit Lasso model as it will reduce the number of variables. But before moving forward, let's check how the ridge model is performing.
...
In order to see the performance of the model, we will be plotting actual versus predicted overall potential values:

```{r fig.align='center', fig.cap='Fig.9:Actual Vs Predicted Values for Ridge Model', message=FALSE, warning=FALSE}

pred_test_ridge_BQ2_1se <- predict(ridge_model_BQ2_1se, newx = fifa_BQ2_test_x)

plot(fifa_BQ2_test_y, pred_test_ridge_BQ2_1se,
     xlab = "Actual Values (Test)",
     ylab = "Predicted Values (Test)",
     main = "Actual vs. Predicted Values for Ridge Model - Test Dataset",
     pch = 20, # Change the point character to a solid circle
     col = "green", # Change the color of the points to green
...
# Add a legend
legend("topleft", legend = "Perfect Prediction", col = "red", lty = 1, cex = 0.8)

# Add a grid
grid()
```
It can be observed that the predicted values are not very far from the actual values and also the distribution of predicted values is random which is a sign of a good model.


Let's now evaluate the Ridge model using RMSE values. RMSE (Root Mean Squared Error) measures the average magnitude of the errors between predicted and actual values, providing a standardized assessment of prediction accuracy. We calculate the RMSE by taking the square root of the average squared differences between predicted and actual values. A lower RMSE indicates better predictive performance, reflecting the model's ability to minimize prediction errors *(Moody,2019)*.
...
table_BQ2_1<-knitr::kable(ridge_BQ2_df, align = "c",
                        caption = "Table 3: RMSE Values for Ridge Regression") %>%
    kable_classic(full_width = FALSE)
table_BQ2_1
...
Lasso regularization, also known as L1 regularization, is a technique used in regression analysis to penalize the absolute size of the coefficients. It adds a penalty term to the regression equation, encouraging the coefficients of less important features to be exactly zero, effectively performing feature selection and improving model interpretability. Lasso regularization helps prevent overfitting and can handle multicollinearity in the dataset *(Manwani,2021)*.

Let's build Lasso model:

```{r fig.align='center', fig.cap='Fig.10:Lasso-Cross Validation Plot', message=FALSE, warning=FALSE}
...
plot(lasso_BQ2)
text(log(lasso_BQ2$lambda.min), max(lasso_BQ2$cvm)/1.2, 
     labels ="log λ.Min",
     pos = 1,srt=90,cex=0.8, col = "red")  

text(log(lasso_BQ2$lambda.1se), max(lasso_BQ2$cvm)/1.2, 
     labels ="log λ.1se", 
     pos = 1, col = "red",cex=0.8, srt=90)
```

It can be observed that the lambda.1se is a better choice as the mean-square root value is low and at the same time it has fewer number of variables as compared to lambda min:

```{r}
lam_las_df<-data.frame(
  "Lambda Min"=Lambda_min_BQ2,
  "Lambda 1se"=Lambda_1se_BQ2
)

table_4<-knitr::kable(lam_las_df, align = "c",digits=4, caption = "Table 4: Lambda Values for Lasso") %>%
    kable_classic(full_width = FALSE)
table_4
```
Let's now build Lasso model using lambda 1se on train dataset:
...
Let's now test the model on test data set and check for Actual vs predicted plot and RMSE values:

```{r fig.align='center', fig.cap='Fig.11:Actual vs. Predicted Values for Lasso ', message=FALSE, warning=FALSE}
...
# Create a scatter plot for Lasso model on the test dataset
plot(fifa_BQ2_test_y, pred_test_BQ2_1se,
     xlab = "Actual Values (Test)",
     ylab = "Predicted Values (Test)",
     main = "Actual vs. Predicted Values for Lasso - Test Dataset",
     pch = 20, # Change the point character to a solid circle
     col = "blue", # Change the color of the points to blue
     cex = 0.7) # Reduce the size of the points
...
grid()
```
From figure 11, it can be observed that predicted values are close to actual values and randomly distributed around the perfectly predicted values which means the model is working well.

Let's now check the RMSE values of Lasso model and compare it with Ridge model:
...
Lasso_BQ2_df_transposed <- t(Lasso_BQ2_df)

# Generate the table
table_5 <- knitr::kable(Lasso_BQ2_df_transposed, align = "c",
                             caption = "Table 5: RMSE Values for Lasso & Ridge Regression") %>%
              kable_classic(full_width = FALSE)
table_5
```

```{r fig.align='center', fig.cap='Fig.12:Comparison of Ridge and Lasso Regularization', message=FALSE, warning=FALSE}

ridge_train <- rmse_train_ridge_BQ2_1se
ridge_test <- rmse_test_ridge_BQ2_1se
lasso_train <- rmse_train_BQ2_1se
lasso_test <- rmse_test_BQ2_1se
...
# Create the bar plot
barplot(
  height = cbind(c(ridge_train, lasso_train), c(ridge_test, lasso_test)),
  beside = TRUE,
  col = colors,
  main = "Comparison of RMSE Values for Ridge and Lasso Regression",
  ylab = "RMSE",
  names.arg = c("Ridge", "Lasso"),
  ylim = c(0, max(c(ridge_train, ridge_test, lasso_train, lasso_test)) * 1.2),
  cex.names = 0.8,
  width = 0.5# Adjust the size of the names
)

# Add legend at the bottom
legend("topright", legend = c("Train", "Test"), fill = colors, inset = c(0, -0.10), xpd = TRUE)
```

**Result of Question 1:** It can be observed that RMSE values are smaller for Lasso model than Ridge model hence we can prefer Lasso model. However, the RMSE value is a little less for test data set than train data set. This could be because the model might be too complex, leading to poor performance on the training set but better performance on the test set. This can happen when the model is trying to fit noise in the training data.In the next step we can try to use subset selection method to build one more model and then compare it to lasso model that has been built.Other way could be removing some of irrelevant numeric variables in the data set and converting few of the relevant categorical variables like performance in different position variables in numeric variables and use them in the model.
...
```{r}
# Load the dataset
fifa_data <- read.csv("outputplayers20.csv")
...
- Songara, A. (2022, March 27). Correlation vs Collinearity vs Multicollinearity. Retrieved from https://medium.com/@songaraankit/correlation-vs-collinearity-vs-multicollinearity-b8e4391617af

- Casati, F. (2022, October 4). Train-Test Data Split. Retrieved from https://medium.com/@sphoebs/train-test-data-split-99b43492cc56

- Moody, J. (2019, September 5). What does RMSE really mean? Towards Data Science. Retrieved from the Towards Data Science website.
https://towardsdatascience.com/what-does-rmse-really-mean-806b65f2e48e

- Manwani, R. (2021, September 20). Lasso and Ridge Regularization – A Rescuer From Overfitting. Analytics Vidhya. Retrieved from https://www.analyticsvidhya.com/blog/2021/09/lasso-and-ridge-regularization-a-rescuer-from-overfitting/
